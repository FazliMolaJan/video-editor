...
public class TextManager {
	...
	public TextManager(Context context, float viewWidth, float viewHeight) {
	...
	public void create() {
		...
		int vertexShaderHandle =
				OpenGLUtil.createShader(GLES20.GL_VERTEX_SHADER, RiGraphicTools.vs_Text);
		int fragmentShaderHandle =
				OpenGLUtil.createShader(GLES20.GL_FRAGMENT_SHADER, RiGraphicTools.fs_Text);
		...
		if (programId != 0)
		{
			...
			GLES20.glBindAttribLocation(programId, 2, "a_texCoord");
			...
			if (linkStatus[0] == 0)
			{
				...
				programId = 0;
				throw new OpenGLException("Failed to create TextManager program.");
			}
		}

		setupImage();
		surfaceChanged(viewWidth, viewHeight);
	}
	...
	private void surfaceChanged(float width, float height) {
		// Clear our matrices
		for(int i=0;i<16;i++) {
		...
		TextureRender.checkGlError("Rectangle programId");
	}
	...
	public void draw() {
		TextureRender.checkGlError("TextManager draw");

		// Set the correct shader for our grid object.
		GLES20.glUseProgram(programId);
		TextureRender.checkGlError("TextManager useProgram");
		...
		TextureRender.checkGlError("TextManager vertexBuffer");
		...
		TextureRender.checkGlError("TextManager textureBuffer");
		...
		TextureRender.checkGlError("TextManager drawListBuffer");

		// get handle to vertex shader's vPosition member
	    int mPositionHandle = GLES20.glGetAttribLocation(programId, "vPosition");
		TextureRender.checkGlError("TextManager vPosition");
	    ...
	    GLES20.glVertexAttribPointer(mPositionHandle, 3,
	                                 ...
	                                 0, vertexBuffer);
		TextureRender.checkGlError("TextManager enable PositionHandle");

	    int mTexCoordLoc = GLES20.glGetAttribLocation(programId, "a_texCoord" );
		TextureRender.checkGlError("TextManager a_texCoord");
		...
		TextureRender.checkGlError("TextManager enable mTexCoordLoc");
		...
		TextureRender.checkGlError("TextManager get uMVPMatrix");

    	// Apply the projection and view transformation
        GLES20.glUniformMatrix4fv(mtrxhandle, 1, false, mtrxProjectionAndView, 0);
		TextureRender.checkGlError("TextManager apply mtrxProjectAndView");

        int mSamplerLoc = GLES20.glGetUniformLocation (programId, "s_texture" );
		TextureRender.checkGlError("TextManager s_texture");

		// Set the sampler texture unit to our selected id
		GLES20.glUniform1i ( mSamplerLoc, 0);
		TextureRender.checkGlError("TextManager sampler");

        // Draw the triangle
        GLES20.glDrawElements(GLES20.GL_TRIANGLES, indices.length, GLES20.GL_UNSIGNED_SHORT,
				drawListBuffer);
		TextureRender.checkGlError("TextManager draw elements");

        // Disable vertex array
        GLES20.glDisableVertexAttribArray(mPositionHandle);
        GLES20.glDisableVertexAttribArray(mTexCoordLoc);
        GLES20.glDisableVertexAttribArray(mColorHandle);
		TextureRender.checkGlError("TextManager disbaled elements");
	...
	private void setupImage() {
		...
		int fontDrawableId = context.getResources().getIdentifier("drawable/font", null,
